...
Indexed.prototype.get = function(key, cb) {
  validateCallback(cb);
  this.backend.get(stringifyKey(key), function(err, val) { cb(err, val) });
};
...
Indexed.prototype.del = function(key, cb) {
  validateCallback(cb);
  this.backend.del(stringifyKey(key), function(err) { cb(err) });
};
...
Indexed.prototype.put = function(key, val, cb) {
  validateCallback(cb);
  this.backend.put(stringifyKey(key), val, function(err) { cb(err) });
};
