...
IDBBackend.prototype.del = function(key, cb) {
  key = this.key(key);
  store('write', cb, function(store, commit) {
...
IDBBackend.prototype.createReadStream = function(options, transform) {
  ...
  store('read', cb, function(store) {
    var start = options.start ? name + ':' + options.start : name;
    var end   = options.end ? name + ':' + options.end : name;
    ...
    request(store.openCursor(range), function(err, e) {
      ...
      if (cursor) {
        var key = cursor.key.replace(name + ':', '');
...
function store(type, cb, fn) {
  getDb(function(err, db) {
    ...
    fn(tr.objectStore('store'), function commit(req, cb) {
      if (req) {
