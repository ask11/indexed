...
IndexedDBBackend.prototype.createReadStream = connect(function(range) {
  var indexName = Object.keys(range).join();
  var that = this;
  var stream = new Stream();
  stream.readable = true;

  this.store.checkIndex(indexName, function(err) {
    if (err) return stream.emit('error', err);

    var condition = createCondition(range);
    var req = that.store('read').index(indexName).openCursor(condition);
...
/**
 * Helper to parse indexed-range to valid IDBKeyRange.
 * https://developer.mozilla.org/en-US/docs/Web/API/IDBKeyRange
 */

function createCondition(range) {
}
