...
IDBStore.prototype.open = function(version, cb) {
  ...
  req.onupgradeneeded = function(event) {
    event.target.result.createObjectStore(that.name, { autoIncrement: false });
  };

  request(req, function(err, event) {
    if (err) return cb(err);

    IDBStore.dbs[that.dbName] = event.target.result;
    that.connectOrUpgrade(event.target.result, cb);
  });
};
